        ...
        public async Task<List<ListAuctioneerDTO>> ListAuctioneer(int status)
        {
            try
            {
                using (var context = new ConnectDB())
                {
                    ...
                    else if (status == 1) // Upcoming auctions (StartDay/StartTime is in the future)
                    {
                        auctioneerList = auctioneerList
                            .Where(a => DateTime.ParseExact(a.StartDay, "yyyy-MM-dd", null) > today ||
                                        (DateTime.ParseExact(a.StartDay, "yyyy-MM-dd", null) == today && TimeSpan.Parse(a.StartTime) > currentTime))
                            .ToList();
                    }
                    else if (status == 2) // Ongoing auctions (StartDay/StartTime has passed but EndDay/EndTime is in the future)
                    {
                        auctioneerList = auctioneerList
                            .Where(a => (DateTime.ParseExact(a.StartDay, "yyyy-MM-dd", null) < today ||
                                         (DateTime.ParseExact(a.StartDay, "yyyy-MM-dd", null) == today && TimeSpan.Parse(a.StartTime) <= currentTime)) &&
                                        (DateTime.ParseExact(a.EndDay, "yyyy-MM-dd", null) > today ||
                                         (DateTime.ParseExact(a.EndDay, "yyyy-MM-dd", null) == today && TimeSpan.Parse(a.EndTime) >= currentTime)))
                            .ToList();
                    }
                    else if (status == 3) // Past auctions (EndDay/EndTime has passed)
                    {
                        auctioneerList = auctioneerList
                            .Where(a => DateTime.ParseExact(a.EndDay, "yyyy-MM-dd", null) < today ||
                                        (DateTime.ParseExact(a.EndDay, "yyyy-MM-dd", null) == today && TimeSpan.Parse(a.EndTime) < currentTime))
                            .ToList();
                    }
        ...
        public async Task AcceptAuctioneerForAdmin(AcceptAutioneerDTO autioneer, string idManager)
        {
            try
            {
                using (var context = new ConnectDB())
                {
                    ...
                    existingAutioneerDetail.PriceStep = autioneer.PriceStep;
